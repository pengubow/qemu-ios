...
static void SHA_Init(SHA1_CTX * context)
{
	...
	switch(offset) {
		case SHA_CONFIG:
			if(value == 0x2 || value == 0xa)
			{
				if(s->memory_mode)
				{
					// we are in memory mode - gradually hash the memory in blocks
					uint8_t *buffer = malloc(0x40);
					for(int i = 0; i < s->insize / 0x40; i++) {
						cpu_physical_memory_read(s->memory_start + i * 0x40, buffer, 0x40);
						SHA1Transform(s->ctx.state, buffer);
	    				memcpy(s->ctx.buffer, buffer, 0x40);
					}
				}
				else
				{
	    			SHA1Transform(s->ctx.state, s->buffer);
	    			memcpy(s->ctx.buffer, s->buffer, 0x40);
				}
    			
    			// update hashout
    			for (int i = 0; i < 20; i++)
			    {
			        s->hashout[i] = ((s->ctx.state[i >> 2] >> ((3 - (i & 3)) * 8)) & 255);
			    }
			} else {
				s->config = value;
			}
